...
const VoiceRecorder = ({ onAudioProcessed }) => {
  ...
  const stopRecording = async () => {
    try {
      ...
      // Send to backend using the updated API endpoint for emergency audio processing
      const result = await AudioService.sendAudioToBackend(API_ENDPOINTS.EMERGENCY);
      
      // Process result based on API format
      if (result && (result.message || result.transcript)) {
        const transcript = result.transcript || result.message;
        onAudioProcessed(transcript);
      } else {
        const errorMessage = result?.error || 'Failed to process audio';
